...
func TestVerifierVerifyConvertsSigstoreResult(t *testing.T) {
	...
	assert.Equal(t, "refs/tags/v1.2.3", verified.Certificate.SourceRef)
	assert.Equal(t, mustDigest(t, "sha1", repeatHex("bb", 20)), verified.Certificate.SourceDigest)
	assert.Equal(t, verification.WorkflowIdentity("https://github.com/owner/repo/.github/workflows/release.yml@refs/heads/main"), verified.Certificate.SignerWorkflow)
