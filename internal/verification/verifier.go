...
func (v *Verifier) VerifyReleaseAsset(ctx context.Context, request Request) (Evidence, error) {
	...
	if validateErr := assetDigest.validate(); validateErr != nil {
		return Evidence{}, wrapError(
			KindDigest,
			validateErr,
			"digest artifact %s returned invalid digest",
			request.AssetPath,
		)
	}
	...
	if validateErr := releaseDigest.validate(); validateErr != nil {
		return Evidence{}, wrapError(
			KindResolveRelease,
			validateErr,
			"resolve release tag %s returned invalid digest",
			request.Tag,
		)
	}
	if !releaseResolution.SourceDigest.IsZero() {
		if validateErr := releaseResolution.SourceDigest.validate(); validateErr != nil {
			return Evidence{}, wrapError(
				KindResolveRelease,
				validateErr,
				"resolve release tag %s returned invalid source digest",
				request.Tag,
			)
		}
	}
	...
	if err != nil {
		return Evidence{}, wrapError(
			KindFetchReleaseAttestations,
			err,
			"fetch release attestations for %s",
			releaseDigest,
		)
	}
	releaseAttestation, err := v.verifyReleaseAttestation(
		ctx,
		releaseAttestations,
		releaseDigest,
		assetDigest,
	...
	if err != nil {
		return Evidence{}, wrapError(
			KindFetchProvenanceAttestations,
			err,
			"fetch provenance attestations for %s",
			assetDigest,
		)
	}
...
func (v *Verifier) verifyReleaseAttestation(
	ctx context.Context,
	...
	if len(attestations) == 0 {
		return VerifiedAttestation{}, newError(
			KindNoReleaseAttestation,
			"no release attestations found for %s",
			releaseDigest,
		)
	}
	...
	for _, attestation := range attestations {
		...
		if !verified.hasTimestampEvidence() {
			fallback = mismatch(
				...
				"release attestation %s has no trusted timestamp evidence",
				attestation.ID,
			)
			continue
		}
		...
		if !verified.Certificate.hasGitHubReleaseIdentity() {
			fallback = mismatch(
				KindReleaseSignerMismatch,
				"release attestation %s has signer identity %q, not %q",
				attestation.ID,
				verified.Certificate.SubjectAlternativeName,
				GitHubReleaseSubjectAlternativeName,
			)
			continue
		}
		...
		if !verified.Statement.hasReleasePredicate() {
			fallback = mismatch(
				KindReleasePredicateMismatch,
				"release attestation %s has predicate %q",
				attestation.ID,
		...
		if verified.Statement.Predicate.ReleaseTag != tag {
			fallback = mismatch(
				KindReleaseTagMismatch,
				"release attestation %s is for tag %s, not %s",
				attestation.ID,
				verified.Statement.Predicate.ReleaseTag,
				tag,
			)
			continue
		}
		...
		if !verified.Statement.hasSubjectDigest(assetDigest) {
			fallback = mismatch(
				KindReleaseSubjectMismatch,
				"release attestation %s does not contain asset digest %s",
				attestation.ID,
				assetDigest,
			)
			continue
...
//nolint:gocognit,funlen // Provenance verification keeps predicate and identity checks in one audited sequence.
func (v *Verifier) verifyProvenanceAttestation(
	ctx context.Context,
	attestations []Attestation,
	assetDigest Digest,
	policy Policy,
	...
	if len(attestations) == 0 {
		return VerifiedAttestation{}, newError(
			KindNoProvenanceAttestation,
			"no provenance attestations found for %s",
			assetDigest,
		)
	}
	...
	for _, attestation := range attestations {
		...
		if !verified.hasTimestampEvidence() {
			fallback = mismatch(
				KindTimestampMissing,
				"provenance attestation %s has no trusted timestamp evidence",
				attestation.ID,
			)
			continue
		}
		...
		if verified.Statement.PredicateType != SLSAPredicateV1 {
			fallback = mismatch(
				KindProvenancePredicateMismatch,
				"provenance attestation %s has predicate %q",
				attestation.ID,
				verified.Statement.PredicateType,
			)
			continue
		...
		if !verified.Statement.hasSubjectDigest(assetDigest) {
			fallback = mismatch(
				KindProvenanceSubjectMismatch,
				"provenance attestation %s does not contain asset digest %s",
				attestation.ID,
				assetDigest,
			)
			continue
		...
		if !verified.Certificate.hasGitHubActionsIssuer() {
			fallback = mismatch(
				KindProvenanceIssuerMismatch,
				"provenance attestation %s has OIDC issuer %q, not %q",
				attestation.ID,
				verified.Certificate.Issuer,
		...
		if err != nil {
			fallback = mismatch(
				KindProvenanceIdentityMismatch,
				"provenance attestation %s has invalid certificate identity %q",
				attestation.ID,
				verified.Certificate.SubjectAlternativeName,
			)
			continue
		...
		if !policy.TrustedSignerWorkflow.matches(subjectWorkflow) {
			fallback = mismatch(
				KindProvenanceIdentityMismatch,
				"provenance attestation %s has certificate identity %q, not %s",
				attestation.ID,
				verified.Certificate.SubjectAlternativeName,
				policy.TrustedSignerWorkflow,
			)
			continue
		}
		...
		if !verified.Certificate.SourceRepository.Equal(policy.ExpectedSourceRepository) {
			fallback = mismatch(
				KindSourceRepositoryMismatch,
				"provenance source repository is %s, not %s",
				verified.Certificate.SourceRepository,
				policy.ExpectedSourceRepository,
			)
			continue
		}
		...
		if !policy.TrustedSignerWorkflow.matches(verified.Certificate.SignerWorkflow) {
			fallback = mismatch(
				KindSignerWorkflowMismatch,
				"provenance signer workflow is %s, not %s",
		...
		if !policy.ExpectedSignerDigest.IsZero() &&
			!verified.Certificate.SignerDigest.equal(policy.ExpectedSignerDigest) {
			fallback = mismatch(
				KindSignerDigestMismatch,
				"provenance signer digest is %s, not %s",
				verified.Certificate.SignerDigest,
				policy.ExpectedSignerDigest,
			)
			continue
		}
		...
		if !policy.ExpectedSourceRef.IsZero() && verified.Certificate.SourceRef != policy.ExpectedSourceRef {
			fallback = mismatch(
				KindSourceRefMismatch,
				"provenance source ref is %s, not %s",
				verified.Certificate.SourceRef,
				policy.ExpectedSourceRef,
			)
			continue
		}
		// If configured, require provenance to come from the exact expected source digest.
		if !policy.ExpectedSourceDigest.IsZero() &&
			!verified.Certificate.SourceDigest.equal(policy.ExpectedSourceDigest) {
			fallback = mismatch(
				KindSourceDigestMismatch,
				"provenance source digest is %s, not %s",
				verified.Certificate.SourceDigest,
				policy.ExpectedSourceDigest,
			)
			continue
		}
		...
		if verified.Certificate.RunnerEnvironment != RunnerEnvironmentGitHubHosted {
			fallback = mismatch(
				KindRunnerEnvironmentMismatch,
				"provenance runner environment is %s, not %s",
				verified.Certificate.RunnerEnvironment,
				RunnerEnvironmentGitHubHosted,
			)
			continue
		}
...
func exactlyOne(
	...
	default:
		return VerifiedAttestation{}, newError(
			KindAmbiguousAttestations,
			"expected one accepted %s, got %d",
			label,
			len(matches),
		)
	}
