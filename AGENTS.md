...
These always-loaded skills are expected to be installed user-globally at `~/.claude/skills/`. If any are missing, tell the user before proceeding with substantive work.
...
- This project explicitly uses Cobra and Viper for CLI command structure, flags, configuration, and environment handling.
- Make extensive use of Charmbracelet libraries to produce a polished terminal UX while preserving a scriptable CLI core.
